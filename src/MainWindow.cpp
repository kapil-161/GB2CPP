    ...
    , m_warningShown(false)
{
    ...
    setStyleSheet(
        ...
        "QTableView { border: 1px solid #C9D6DF; gridline-color: #E4E8ED; }"
        "QTableView::item:selected { background-color: #A8D8F0; color: #000000; }"
...
void MainWindow::setupControlPanel()
{
    ...
    m_fileListWidget->setToolTip("Select output files to plot.\nDrag & drop .OUT, .OSU, .OLC, .OPG, .OVT, .OPT, .CSV files from Explorer to add them.");
...
<ul>
  ...
  <li>Supported file types: <code>.OUT</code>, <code>.OSU</code>, <code>.OLC</code>, <code>.OPG</code>, <code>.CSV</code>, <code>EVALUATE.OUT</code>, and observed T files (<code>.WHT</code>, <code>.MZT</code>, <code>.SOT</code>, …).</li>
  ...
  <li><b>Drag &amp; drop</b> — drag one or more DSSAT output files from Windows Explorer directly onto the outfile list.
    Supported extensions: <code>.OUT</code>, <code>.OSU</code>, <code>.OLC</code>, <code>.OPG</code>, <code>.OVT</code>, <code>.OPT</code>, <code>.CSV</code>, all other O-extension files, and <b>observed T files</b> (<code>.WHT</code>, <code>.MZT</code>, <code>.SOT</code>, etc.).
    <ul>
